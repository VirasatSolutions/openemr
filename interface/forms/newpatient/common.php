                            ...
                            <div class="form-group ">
                                ...
                                <div class="form-group">
                                    ...
                                    <div
										...
										<?php
											if($viewmode && $result['authorization'] == 1){
												$disabled = '';
												$checked = 'checked';
											}else{
												$checked = '';
										...
										?>
										<label class="control-label col-sm-2 oe-text-to-right"><?php echo xlt('Authorization Code:'); ?> &nbsp;</label>
										<div class="col-sm-3">
											<input type='text' class='form-control authorization_code col-sm-12' name='authorization_code' id='authorization_code' value='<?php echo $viewmode ? $result['authorization_code'] : ''; ?>' <?php echo $disabled; ?>/>
										</div>
										<div class="col-sm0">
											<input type="checkbox" name="authorization" id='authorization' value="" <?php echo $checked; ?>>
										</div>
                                ...
                                <div class="form-group">
                                    <label  class="control-label col-sm-2 oe-text-to-right"><?php echo xlt('Encounter No'); ?>:</label>
									<div class="col-sm-3">
										<select class='form-control col-sm-12' name='encounter_no'>
										<?php
											for($i=1;$i<=10;$i++){
												
												$data = $viewmode ? $result['encounter_no'] : $numOfRows;
									...
									<label for='form_onset_date' class="control-label col-sm-2 oe-text-to-right"><?php echo xlt('Expiration Date:'); ?> &nbsp;</label>
									<div class="col-sm-3">
										<input type='text' class='form-control datepicker col-sm-12' name='form_onset_date' id='form_onset_date' value='<?php echo $viewmode && $result['onset_date']!='0000-00-00 00:00:00' ? attr(oeFormatShortDate(substr($result['onset_date'], 0, 10))) : ''; ?>' />
									</div>
									<div class="clearfix"></div>
                                </div>
                                <div class="form-group"
								...
								<div class="form-group">
									<label class="control-label col-sm-2 oe-text-to-right"></label>
									<div class="col-sm-8">
										<a class="super_bill css_button_small form-edit-button col-sm-4" href=""><span>Superbill</span></a>
									</div>
								</div>
                        </div>
                        <?php if ($GLOBALS['set_pos_code_encounter']) { ?>
                        ...
                        <legend><?php echo xlt('Reason for Visit')?></legend>
                            <div class="form-group">
                                <div class="col-sm-10 col-sm-offset-1">
                                    <!--<textarea name="reason" id="reason" class="form-control" cols="80" rows="4" ><?php echo text($result['reason']); ?></textarea>-->
									
									<select class='form-control col-sm-12' name='reason'>
										<option value="_blank">-- Select One --</option>
										<option value='PT' <?php if($result['reason'] == 'PT'){echo 'selected';} ?>>PT</option>
										<option value='OT' <?php if($result['reason'] == 'OT'){echo 'selected';} ?>>OT</option>
										<option value='SLP' <?php if($result['reason'] == 'SLP'){echo 'selected';} ?>>SLP</option>
									</select>
                                    ...
                                    <div class="col-sm-6 col-lg-offset-3">
                                            <div class="col-sm-12">
                                                <div class="col-sm-12 pull-left" style="padding-bottom:5px">
													...
													<?php
														$sqlCount = sqlStatement("SELECT e.encounter, e.date, c.pc_catname FROM form_encounter as e LEFT JOIN openemr_postcalendar_categories as c ON c.pc_catid = e.pc_catid WHERE e.pid = $pid ORDER BY e.id DESC");
														$i=0;
														$li = '';
														while($row = sqlFetchArray($sqlCount)) {
															$i++;
															$html = '<div class="btn-group dropdown">
																<button class="btn btn-default btn-sm dropdown-toggle" type="button" id="pastEncounters" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
																	Copy from old visit&nbsp;
																	(<span data-bind="text:encounterArray().length">'.$i.'</span>)
																	<span class="caret"></span>
																</button>';
															$li .= '<li><a class="encounter" data-id="'.$row['encounter'].'"">
																	<span>'.$row['date'].'</span>
																	<span>'.$row['pc_catname'].'</span>
																</a></li>';
														}
														echo $html;
														echo "<ul class='dropdown-menu'>$li</ul></div>";
													?>
                                                </div>
                                                <div class="col-sm-12 pull-left">
                                                    <select multiple name='issues[]' id='encounter_issues' class='col-sm-12'
...
$(function (){
	...
	$('.super_bill').click(function() {
		var from = $(".datepicker[name=form_date]").val();
		var onset = $(".datepicker[name=form_onset_date]").val();
		$($(this)).attr("href", "<?php echo $GLOBALS['webroot']?>/interface/patient_file/printed_fee_sheet.php?fill=1&form_date="+from+"&onset_date="+onset);
	});
});
...
$('#authorization').change(function() {
	if(this.checked) {
		$(this).prop("checked");
		$("#authorization_code").attr('disabled',false);
	}else{
		$("#authorization_code").attr('disabled',true);
	}    
});

$('.encounter').click(function() {
	var id = $(this).data("id");
	var url = '<?php echo $GLOBALS['webroot']?>/interface/forms/newpatient/ajax.php';
	$.ajax({
		url: url,
		data: { encounter: id, 'pid': <?php echo $pid; ?> }
	}).done(function(res) {
		$('#encounter_issues').empty().append(res);
	});
});
</script>
</html>
