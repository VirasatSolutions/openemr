...
<td colspan='3' valign='top' class='fshead' style='height:${lheight}pt'>";
            ...
            $encdata = false;
            if ($form_fill && $form_date && $onset_date) {
                $query = "SELECT fe.reason, fe.date, u.fname, u.mname, u.lname, u.username " .
                        ...
                        "WHERE f.pid = ? AND f.formdir = 'newpatient' AND f.deleted = 0 " .
                        " AND fe.date >= ? AND fe.onset_date <= ? ORDER BY f.id DESC LIMIT 1";
                $encdata = sqlQuery($query, array($pid, $form_date, $onset_date));
