...
$searchAny = empty($searchAny) ? $sSearch : $searchAny;
...
$iTotal = $row['count'];
if($iTotal < 1){
	$row = sqlQuery("SELECT COUNT(id) AS count FROM insurance_data WHERE claim_number LIKE '$searchAny%'");
	$iTotal = $row['count'];
	$claimnumber = TRUE;
}
...
if($claimnumber){
	$query = "SELECT p.pid, p.lname, p.fname, p.mname, p.phone_home, p.ss, p.DOB, p.pubpid FROM patient_data as p LEFT JOIN insurance_data as i ON p.id = i.pid WHERE i.claim_number LIKE '$searchAny%' GROUP BY p.pid";
...
while ($row = sqlFetchArray($res)) {
	...
	$query = sqlStatement("SELECT claim_number FROM insurance_data WHERE pid = $row[pid] AND claim_number IS NOT NULL");
	$claim_number = '';
	if(!empty($query)){
		while ($insurance_data = sqlFetchArray($query)) {
			$claim_number .= $insurance_data['claim_number'].', ';
		}
		$claim_number = rtrim($claim_number, ', ');
	}
    foreach ($aColumns as $colname) {
        if ($colname == 'claim_number') {
			$arow[] = $claim_number;
